...
channels_to_watch = [
    -1001982472141,  # کانال اول
    -1002468383817   # کانال دوم
]
...
def clean_line(line):
    # فقط کاراکترهای غیرمجاز رو حذف کن، پرانتز و خط تیره و نقطه و ... رو نگه دار
    return line.strip()
...
def extract_fields_channel2(text):
    ...
    # مرحله 1: استخراج با regex استاندارد
    try:
        # Pair و Position
        ...
        # Entry (سه مرحله)
        entry = '-'
        # مرحله 1: عدد اعشاری داخل پرانتز
        m = re.search(r'KATEX_INLINE_OPEN([\d\.]+)KATEX_INLINE_CLOSE', lines[1])
        if m:
            entry = m.group(1)
        # مرحله 2: اولین عدد اعشاری در کل خط
        if entry == '-':
            m = re.search(r'(\d+\.\d+)', lines[1])
            if m:
                entry = m.group(1)
        # مرحله 3: split با فاصله و گرفتن اولین عدد اعشاری
        if entry == '-':
        ...
        # TP (سه مرحله)
        tps = []
        # مرحله 1: همه اعداد اعشاری داخل پرانتز و جدا شده با -
        ...
        # مرحله 2: همه اعداد اعشاری در کل خط
        if not tps:
            tps = re.findall(r'\d+\.\d+', lines[2])
        # مرحله 3: split با فاصله و گرفتن همه اعداد اعشاری
        if not tps:
        ...
        # SL (سه مرحله)
        sl = '-'
        # مرحله 1: عدد اعشاری داخل پرانتز بعد از SL
        ...
        # DCA (سه مرحله)
        dca = '-'
        # مرحله 1: عدد اعشاری داخل پرانتز بعد از DCA
        m = re.search(r'DCA\s*=\s*KATEX_INLINE_OPEN([\d\.]+)KATEX_INLINE_CLOSE', lines[3])
        if m:
            dca = m.group(1)
        # مرحله 2: دومین عدد اعشاری در خط (اگر SL و DCA در یک خط باشند)
        if dca == '-':
            nums = re.findall(r'\d+\.\d+', lines[3])
            if len(nums) > 1:
                dca = nums[1]
        # مرحله 3: split با فاصله و گرفتن دومین عدد اعشاری
